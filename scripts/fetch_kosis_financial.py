#!/usr/bin/env python3
"""Build data/kosis_financial.json (시장 지표 하위: 금융) from KOSIS snapshots.

Covers PROJECT_SPEC.md 섹션 3-1 subcategories: 시장금리 / 가계수신 / 카드소비.
기업대출 has no matching indicator in the KOSIS 100대 지표 catalog (그 항목은
ECOS 연동 전까지 프런트에서 "준비중" 카드로 표시). Raw series fetched via the
koreaStat MCP tools on 2026-07-05.

TODO (섹션 0/5-2): same caveat as fetch_kosis.py — this is a hand-fetched
snapshot, not yet a live GitHub Actions + KOSIS Open API pipeline.
"""
import json
import os
...
ORDER = ["1202", "1203", "669", "686"]

# 스펙 3-1 서브카테고리 표시 순서 (기업대출은 데이터 없음 -> 프런트에서 준비중 카드)
SUBCATEGORIES = ["시장금리", "가계수신", "기업대출", "카드소비"]
