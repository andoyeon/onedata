...
| 카테고리 | 소스 | 연동 방식 | 갱신 주기 |
|---|---|---|---|
| 시장 지표 | KOSIS (통계청) | KOSIS Open API, 서비스키 발급 필요 | 항목별 상이 (월/분기) |
| 시장 지표 | DART (금감원 전자공시) | DART Open API, 서비스키 발급 필요 | 공시 발생 시 |
| 시장 지표(추후) | 한국은행 ECOS | 지금은 제외. 추후 자체 MCP 개발 후 카카오 등록 예정 | - |
| 외국인 지표 | 법무부(출입국·외국인정책본부) | **공공데이터포털(data.go.kr) Open API** — hikorea.go.kr 직접 크롤링 금지 | 연보(6월말)/월보(매월) |
...
/onedata
  ...
  /scripts
    ...
    build_catalog.py
  /.github/workflows
    update-market.yml        # KOSIS/DART 수집 스케줄
...
### 3-1. 시장 지표 페이지
- 서브 카테고리 필터 칩: 시장금리 / 가계수신 / 기업대출 / 카드소비
  - 지금은 ECOS 미연동이므로 커버 안 되는 항목은 "준비중" 카드로 자리만 표시
...
- 출처 고지 문구 필수: "법무부 출입국·외국인정책본부(공공데이터포털 제공)"
...
2. 기존 KOSIS 파이프라인 구조 확인 후, 동일 패턴으로 `fetch_dart` 스크립트 및 Actions 워크플로 추가 → 시장 지표 페이지 완성
3. data.go.kr 법무부 데이터셋 연동 스크립트 작성 → 외국인 지표 페이지 완성
4. `build_catalog.py`로 카탈로그 JSON 생성 → 검색 페이지 구현
5. (추후) ECOS 직접 API 연동 → 이후 MCP 서버로 감싸서 카카오 PlayMCP 등록 신청
...
```
PROJECT_SPEC.md 문서를 읽고, 섹션 2(리포지토리 구조)에 맞춰 폴더를 구성하고
섹션 5의 순서 1번(디자인 토큰 + 공통 컴포넌트)부터 구현해줘.
기존 KOSIS 관련 코드는 유지하되, data 폴더 구조에 맞게 필요하면 리팩터링해줘.
```
